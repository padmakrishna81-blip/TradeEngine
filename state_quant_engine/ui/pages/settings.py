...
def render(settings: Any, version_id: int = 1) -> None:
    ...
    with tab_rules:
        ...
        st.info(
            "**BUY/EXIT/AVG thresholds and Hard Gates** are now configured per asset type in "
            "**Scoring Profiles → Profile Settings**. "
            "The fields below are global rules that apply regardless of profile."
        )

        # ── Hard Stop-Loss (global — not per-profile) ─────────────────────
        st.write("### 🛑 Hard Stop-Loss")
        st.caption("Immediate EXIT regardless of health score or profile thresholds.")
        ...
        with hs1:
            hard_stock = st.number_input(
                ...
                help="Stocks: EXIT immediately if P&L drops below this %",
            )
        with hs2:
            hard_etf = st.number_input(
                ...
                min_value=-30.0, max_value=0.0, step=0.5,
                help="ETFs: EXIT immediately if P&L drops below this %",
            )
        ...
        # ── Trailing buffers (global) ──────────────────────────────────────
        st.write("### 📉 Dynamic Trailing Stop Buffers")
        st.caption("Trailing stop = highest_profit − buffer. Lower health → tighter buffer.")
